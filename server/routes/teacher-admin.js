...
teacherAdminRouter.get('/overview', (req, res) => {
  ...
  const students = req.user.role === 'admin'
    ? db.prepare(`SELECT id,username,email,role,xp,level,coins FROM users WHERE role='student' ORDER BY username`).all()
    : db.prepare(`SELECT DISTINCT u.id,u.username,u.email,u.role,u.xp,u.level,u.coins FROM users u JOIN class_members cm ON cm.user_id=u.id JOIN classes c ON c.id=cm.class_id WHERE c.teacher_id=? ORDER BY u.username`).all(req.user.id);
...
teacherAdminRouter.get('/classes/:classId/students', (req,res) => {
  ...
  const students = db.prepare(`SELECT u.id,u.username,u.email,u.xp,u.level,u.coins,cm.joined_at FROM class_members cm JOIN users u ON u.id=cm.user_id WHERE cm.class_id=? AND u.role='student' ORDER BY u.username`).all(classId);
  res.json({students});
});

teacherAdminRouter.post('/students/move', (req,res) => {
  const parsed=z.object({userId:z.coerce.number().int().positive(),fromClassId:z.coerce.number().int().positive().nullable().optional(),toClassId:z.coerce.number().int().positive()}).safeParse(req.body);
  if(!parsed.success) return res.status(400).json({error:'Données invalides'});
  const {userId,fromClassId,toClassId}=parsed.data;
  if(!canManageClass(req,toClassId) || (fromClassId && !canManageClass(req,fromClassId))) return res.status(403).json({error:'Classe inaccessible'});
  const student=db.prepare("SELECT id,username FROM users WHERE id=? AND role='student'").get(userId);
  if(!student) return res.status(404).json({error:'Élève introuvable'});
  const move=db.transaction(()=>{
    if(fromClassId) db.prepare('DELETE FROM class_members WHERE class_id=? AND user_id=?').run(fromClassId,userId);
    db.prepare('INSERT OR IGNORE INTO class_members(class_id,user_id) VALUES(?,?)').run(toClassId,userId);
  });
  move();
...
teacherAdminRouter.post('/classes', (req,res) => {
  ...
  if(!teacher) return res.status(400).json({error:'Professeur invalide'});
...
teacherAdminRouter.post('/catalog/items', (req,res) => {
  ...
  try {
    ...
    res.status(201).json({ok:true,id:result.lastInsertRowid,slug:parsed.data.slug});
  } catch(error){
    if(String(error.message).includes('UNIQUE')) return res.status(409).json({error:'Ce slug existe déjà'});
    console.error(error); res.status(500).json({error:'Création impossible'});
  }
});
