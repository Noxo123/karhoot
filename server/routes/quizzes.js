...
const schema = z.object({ title:z.string().min(2).max(100), description:z.string().max(500).optional(), questions:z.array(z.object({ question:z.string().min(1), points:z.number().int().min(1).max(1000).default(100), answers:z.array(z.object({ answer:z.string().min(1), isCorrect:z.boolean() })).min(2).max(8) })).min(1).max(100) });
...
quizRouter.get('/:id', requireAuth, (req,res) => {
  const quiz=db.prepare('SELECT id,title,description,visibility FROM quizzes WHERE id=?').get(req.params.id); if(!quiz)return res.status(404).json({error:'Quiz introuvable'});
  quiz.questions=db.prepare('SELECT id,question,points,order_index FROM questions WHERE quiz_id=? ORDER BY order_index').all(quiz.id).map(q=>({...q,answers:db.prepare('SELECT id,answer,is_correct FROM answers WHERE question_id=? ORDER BY order_index').all(q.id)})); res.json({quiz});
});
