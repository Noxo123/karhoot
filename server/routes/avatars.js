...
function getConfig(userId) {
  ...
  try {
    ...
    return { character:raw.character||'lorelei-default', outfit:raw.outfit||'', background:raw.background||'', seed:String(raw.seed||row?.username||'karhoot').slice(0,80), options:raw.options&&typeof raw.options==='object'&&!Array.isArray(raw.options)?raw.options:{} };
  } catch { return {character:'lorelei-default',outfit:'',background:'',seed:row?.username||'karhoot',options:{}}; }
}

avatarRouter.get('/catalog', requireAuth, (req,res) => {
  const rows = db.prepare('SELECT id,slug,name,category,style,price,description,icon FROM avatar_items WHERE active=1 ORDER BY category,price,id').all();
  const items = rows.map(item => ({ ...item, ...(catalogMeta.get(item.slug)?.optionKey ? { optionKey:catalogMeta.get(item.slug).optionKey, optionValue:catalogMeta.get(item.slug).optionValue } : {optionKey:'',optionValue:''}), owned:item.price===0||owned(req.user.id,item.slug), equipped:false }));
  const config=getConfig(req.user.id);
  for(const item of items){
    item.equipped = [config.character,config.outfit,config.background].includes(item.slug) || (item.optionKey && String(config.options?.[item.optionKey])===String(item.optionValue));
  }
  res.json({items});
});
...
avatarRouter.post('/purchase', requireAuth, (req,res) => {
  ...
  try { const coins=buy(); res.json({ok:true,coins,item}); }
